...
export default function AgencyDashboard({ user, onLogout, apiUrl }) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/agency/dashboard`, {
        headers: {
      ...
      setDashboardData({
        kpis: {
          total_clients: 47,
          monthly_revenue: 67890,
          avg_roi: 320,
          active_campaigns: 156
        },
        plans_performance: [
          { plan: 'starter', clients: 18, revenue: 5346 },
          { plan: 'aceleracao', clients: 22, revenue: 13134 },
          { plan: 'exponencial', clients: 7, revenue: 9079 }
        ],
        top_companies: [
          { name: 'TechStart Ltda', plan: 'exponencial', roi: 387 },
          { name: 'Marketing Pro', plan: 'aceleracao', roi: 342 },
          { name: 'Vendas Plus', plan: 'aceleracao', roi: 298 }
        ]
      })
    } finally {
      setLoading(false)
