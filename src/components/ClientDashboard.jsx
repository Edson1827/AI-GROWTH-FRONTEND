...
export default function ClientDashboard({ user, onLogout, apiUrl }) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Use mock data based on user plan
      const plan = user.company?.plan || 'starter'
      setDashboardData({
        kpis: {
          roi: plan === 'starter' ? 280 : plan === 'aceleracao' ? 320 : 350,
          revenue: plan === 'starter' ? 8400 : plan === 'aceleracao' ? 19140 : 45395,
          campaigns: plan === 'starter' ? 5 : plan === 'aceleracao' ? 15 : 25,
          conversions: plan === 'starter' ? 142 : plan === 'aceleracao' ? 387 : 892
        },
        campaigns: [
          { name: 'Google Ads - Vendas', status: 'active', roi: 340, budget: 1500 },
          { name: 'Facebook - Leads', status: 'active', roi: 285, budget: 800 },
          { name: 'Instagram - Branding', status: 'paused', roi: 195, budget: 600 }
        ],
        automations: plan === 'starter' ? 3 : plan === 'aceleracao' ? 8 : 15
      })
    } finally {
      setLoading(false)
    }
